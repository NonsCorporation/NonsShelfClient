...
import { IoStar, IoFlame, IoPeopleOutline, IoLogInOutline } from 'react-icons/io5'
...
export default function DiscoverPage() {
  ...
  // Catalog rows for the active scope. Refetch when the toggle changes; the
  // public /api/media endpoint answers anonymously.
  useEffect(() => {
    let cancelled = false
    setLoading(true)
    Promise.all([
    ...
    <Layout>
      ...
      {loading ? (
        ...
        <div className="animate-fade-up">
          ...
          <Row title={t('newAndNoteworthy')} items={recent} inLibrary={inLibrary} addProp={addProp} />
...
// signed-in users (undefined ⇒ no add button).
function Row({
  title, items, showReason, inLibrary, addProp,
}: {
  title: string
  items: CatalogItem[]
  showReason?: boolean
