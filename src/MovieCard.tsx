...
export default function MovieCard({ title, director, actors, genre, year, duration, coverUrl }: MovieCardProps) {
    return (
        <div className="w-48 h-[340px] bg-nonscontainerbg rounded-sm group overflow-hidden cursor-pointer flex flex-col border border-white/5 hover:bg-white/5 hover:border-white/10 transition-all duration-300">
            ...
            <div className="relative w-full aspect-[2/3] overflow-hidden rounded-sm bg-gray-800 shadow-md">
                ...
                {/* year badge (top-left) */}
                {year && (
                    <div className="absolute top-2.5 left-2.5 z-30 bg-black/70 backdrop-blur-md px-1.5 py-0.5 rounded text-[11px] font-medium text-white/90 shadow-sm border border-white/10">
                        {year}
                    </div>
                )}
            ...
            <div className="p-3 flex flex-col gap-1 flex-none">
                <h2 className="text-[15px] font-semibold text-gray-100 leading-snug truncate" title={title}>
                    {title}
                </h2>
                {director && (
